        ...
        public static void InsereFaturamentos(Faturamento faturamento)
        {
            ...
            try
            {
                if (Consulta.VerificaExistencia("invoice", "invoicenumber", faturamento.invoicenumber.ToString()))
                {
                    ...
                    {
                        entidade["ownerid"] = Consulta.Busca("systemuser", "fullname", faturamento.ownerid.ToString());
                    ...
                    try
                    {
                        ...
                        if (!String.IsNullOrEmpty(faturamento.quantity.ToString()))
                            produtoFatura["quantity"] = Convert.ToDecimal(faturamento.quantity.ToString().Replace(",", "").Replace(".", ","));
                        if (!String.IsNullOrEmpty(faturamento.priceperunit.ToString()))
                            produtoFatura["priceperunit"] = new Money(Convert.ToDecimal(faturamento.priceperunit.ToString().Replace(".", ",")));
                        ...
                        if (!String.IsNullOrEmpty(faturamento.tax.ToString()))
                            produtoFatura["tax"] = Convert.ToDecimal(faturamento.tax);
