...
namespace Integracao
{
    static class Program
    {
        ...
        static void Main()
        {
            ...
            foreach (System.IO.FileInfo f in di.GetFiles())
            {
                if (f.Extension.ToLower().Equals(".xlsx") || f.Extension.ToLower().Equals(".csv"))
                {
                    ...
                    {
                        FaturamentosExcel(newFile);
                    }

                    if (f.FullName.Contains("grupoprod"))
                    {
                        GrupoProdutosExcel(newFile);
                    }

                    if (f.FullName.Contains("produto"))
                    {
                        ProdutosExcel(newFile);
                    }
                }
            }*/
        }
        ...
        /*static void ProdutosExcel(FileInfo newFile)
        {
            try
            {
                using (ExcelPackage package = new ExcelPackage(newFile))
                {
                    ...
                    {
                        Produtos prod = new Produtos();
                        prod.productnumber = sheet.Cells[row, 1].Value.ToString().Trim(); //childNode["b1_cod"].InnerText;
                        prod.name = sheet.Cells[row, 2].Value.ToString().Trim(); //childNode["b1_desc"].InnerText;
                        prod.results_grupodoprodutoid = sheet.Cells[row, 3].Value.ToString().Trim(); //childNode["b1_grupo"].InnerText;
                        prod.parentproductid = sheet.Cells[row, 4].Value.ToString().Trim(); //childNode["b1_ativid"].InnerText;
                        prod.results_embalagem = sheet.Cells[row, 5].Value.ToString().Trim(); //childNode["b1_embalagem"].InnerText;
                        prod.defaultuomid = sheet.Cells[row, 7].Value.ToString().Trim(); //childNode["b1_um"].InnerText;
                        prod.defaultuomscheduleid = "Unidade Padrão";
                        prod.statecode = sheet.Cells[row, 10].Value.ToString().Trim(); //childNode["b1_msblql"].InnerText;
            ...
            {
                Insercoes.InsereLog("IntegracaoProdutos", false, "Produto", e.Message);
            }
        }*/
        ...
        /*static void GrupoProdutosExcel(FileInfo newFile)
        {
            try
            {
                using (ExcelPackage package = new ExcelPackage(newFile))
                {
                    ...
                    while (row <= sheet.Dimension.Rows)
                    {
                        ...
                        prod.results_name = sheet.Cells[row, 1].Value.ToString().Trim(); //childNode["bm_desc"].InnerText;
                        prod.results_codigo = sheet.Cells[row, 2].Value.ToString().Trim(); //childNode["bm_grupo"].InnerText;
              ...
              //Pegando elemento pelo nome da TAG
                ...
                foreach (XmlNode xn in xnList)
                {
                    foreach (XmlNode childNode in xn.ChildNodes)
                    {
                        ...
                        fat.tax = childNode["d2_icmsret"].InnerText; 
                        ...
                        if (fat.ownerid.ToString() == "DEVOLUCAO")
                        {
                            Insercoes.AtualizaDevolucao(fat);
                        }
                        else
                        {
        ...
        /*static void FaturamentosExcel(FileInfo newFile)
        {
            try
            {
                using (ExcelPackage package = new ExcelPackage(newFile))
                {
                    ...
                    while (row <= sheet.Dimension.Rows)
                    {
                        ...
                        fat.tax = sheet.Cells[row, 32].Value.ToString().Trim(); //childNode["x_impostos"].InnerText;
