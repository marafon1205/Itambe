   ...
   public static class Consulta
    {
        ...
        public static EntityCollection VerificaExistenciaDevolucao(string nomeEntidade, string nomeCondicao, string valorCondicao)
        {
            QueryExpression query = new QueryExpression()
            {
                EntityName = nomeEntidade,
                ColumnSet = new ColumnSet(new string[] { "ownerid", "invoicenumber" })
            };
            query.Criteria.AddCondition(nomeCondicao, ConditionOperator.Equal, new object[] { valorCondicao });
            return Consulta.service.RetrieveMultiple(query);
        }

        public static EntityCollection VerificarProdutoFatura(Entity nomeEntidade)
        {
            QueryExpression query = new QueryExpression("invoicedetail");
            query.Criteria.AddCondition("invoiceid", ConditionOperator.Equal, new object[] { nomeEntidade.Id });
            return Consulta.service.RetrieveMultiple(query);
        }
